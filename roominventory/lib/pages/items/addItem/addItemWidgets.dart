...
class LocationSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    String getSelectedPlaceName() {
      if (controller.selectedPlace == null) return 'Select';
      final place = controller.places.firstWhere(
        (p) =>
            (p['idplace']?.toString() ?? p['IdPlace']?.toString() ?? '') ==
            controller.selectedPlace,
        orElse: () => {'placename': 'Unknown', 'PlaceName': 'Unknown'},
      );
      return place['placename']?.toString() ??
          place['PlaceName']?.toString() ??
          'Unknown';
    }

    String getSelectedZoneName() {
      if (controller.selectedZone == null) return 'Select';
      final zone = controller.filteredZones.firstWhere(
        (z) =>
            (z['idzone']?.toString() ?? z['IdZone']?.toString() ?? '') ==
            controller.selectedZone,
        orElse: () => {'zonename': 'Unknown', 'ZoneName': 'Unknown'},
      );
      return zone['zonename']?.toString() ??
...
class RotatingDatePickerDialog extends StatelessWidget {
  final AddItemController controller;
  final Function(DateTime) onDateChanged;

  const RotatingDatePickerDialog({
    Key? key,
    required this.controller,
